//! Nested-call lint ported from laroccacharly/mask `tests/no_nested_calls.rs`.
//! Function calls must be named before being passed as arguments.
...
use syn::{Expr, ExprCall, ExprMethodCall, ItemMod};
...
impl<'ast> Visit<'ast> for NestedCallVisitor {
    ...
    fn visit_expr_call(&mut self, node: &'ast ExprCall) {
        if self.skip_depth == 0 && !is_constructor(&node.func) {
...
fn function_calls_are_named_before_being_passed_to_other_calls() {
    ...
    sources.push(root.join("../cli/src"));
    sources.push(root.join("../lambda/src"));
